...
antcol <- dat$Colony[1:20]
antmas <- dat$Mass[1:20]
dat3 <- data.frame(Colony = antcol, Mass = antmas)
...
write.csv(dat3, file = "JENSEN_first_file.csv")
...
#fill it in
size_class_mean_mass = data.frame(FLevel=levels(dat$Size.class), Mean=c(new_vector))
...
jpeg(filename = "Ant_HeadvMass.jpeg")
plot(dat$Headwidth, dat$Mass, col = dat$Colony, main = "Headwidth v. Mass", xlab = "Headwidth", ylab = "Mass")
dev.off
?jpeg
# 3.  Subset the thatch ant data set to only include ants from colony 1 and colony 2
col1_and_2 <- dat[dat$Colony == c("1","2"),]

# 4.  Write code to save this new subset as a .csv file
write.csv(col1_and_2, file = "colonies1and2.csv")
