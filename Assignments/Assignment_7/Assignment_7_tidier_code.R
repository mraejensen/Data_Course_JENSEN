...
buddhist_long <- gather(buddhist, key=Religion, value = Proportion, -c(1:3))
...
utah%>%
  group_by(Pop.Group)%>%
  summarize(MeanReligiosity = mean(Religious))
...
utah%>%
  group_by(Pop.Group)%>%
  summarize(meanPop = mean(Pop_2010))
...
religiosity <- utah%>%
  group_by(Pop.Group)%>%
  summarize(Mean.Religiosity = mean(Religious), Mean.Pop = mean(Pop_2010))
